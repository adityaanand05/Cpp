...
int main() {
    string teatypes[] = {
        "Green Tea", "Black Tea", "Herbal Tea", "Oolong Tea", "No Tea", "White Tea"
    };

    int size = sizeof(teatypes) / sizeof(teatypes[0]);
    ...
    for (int i = 0; i < size; ++i) {
        ...
        if (teatypes[i] == "No Tea") {
            cout << "No Tea found. Stopping service.\n";
            break;
        }
...
// Write a program that displays a tea brewing schedule. 
//For each day of the week (outer loop), brew 2 cups of tea for breakfast, lunch, and dinner (inner loop).
...
int main() {
    ...
    for (int i = 1; i <= days; i++)
    {
        ...
        for (int j = 0; j < 3 ; j++)
        {
           cout << "Brewing cups of tea for " <<foodtime[j]<<endl;
           for (int t = 1; t<=2;t++)
           {
                cout << "Brewin "<<t<<" cups of tea\n";
           }
